...
class Harapartners_Import_Adminhtml_ImportController extends Mage_Adminhtml_Controller_action {
	...
	public function indexAction() {
		$this->_initAction()
			->_addContent($this->getLayout()->createBlock('import/adminhtml_import'))
			->renderLayout();
	}
	...
	public function saveAction() {
		...
		if(!!Mage::helper('hpvendor/data')->getAdminVendorId()){
			$data['vendor_id'] = Mage::helper('hpvendor/data')->getAdminVendorId();
		}
		...
		try {
			...
			if($shouldRunImport){
				try{
					...
					Mage::getSingleton('adminhtml/session')->addError(Mage::helper('import')->__('There is an error processing the uploaded data. ' . $e->getMessage()));
				}
