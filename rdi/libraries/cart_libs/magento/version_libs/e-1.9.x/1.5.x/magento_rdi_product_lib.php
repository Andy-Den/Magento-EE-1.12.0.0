    ...
    //{ 
        ...
        {                                     
            ...
            update_product_attribute_field_table($product_data['entity_id'], $attribute_data['type'], $website_ids[0], array($attribute_data['attribute_id'] => $product_data[$field_for_update]));
                                                ...
                                                "is_in_stock", "low_stock_date", "use_config_notify_stock_qty", "use_config_manage_stock", "stock_status_changed_automatically", "use_config_qty_increments",
...
//        }
    ...
    $sql = "REPLACE INTO cataloginventory_stock_item (qty, 
                                              ...
                                              values (
                                                      ...
                                                      " . (!isset($product_data['stock_status_changed_automatically']) ? '1' : $product_data['stock_status_changed_automatically']) . ", 
...
function set_visibility_status_array($product_array)
{
    ...
    if(is_array($product_array))
    {
        ...
        {
            #temparily set rdi_deactivated_date to 12/31/3000
            $sql = "replace into catalog_product_entity_datetime (entity_type_id, attribute_id, store_id, entity_id, value) values({$product_entity_type_id}, {$deactivated_date_attribute_id}, " . (isset($store_id) ? $store_id : 0) . ", {$row['product_id']}, '3000-12-31 14:29:14')";
            $cart->get_db()->exec($sql);
...
{
    global $hide_out_of_stock, $cart, $disable_out_of_stock, $store_id, $pos, $field_mapping;
...
function insert_product_stock_entity($product_entity_id, $product_data)
{
    ...
    $sql = "INSERT INTO `cataloginventory_stock_item` (`product_id`, `stock_id`,
                                                ...
                                                `stock_status_changed_automatically`, `use_config_qty_increments`, `use_config_enable_qty_inc`, manage_stock) 
                                             VALUES 
                                             (
                                                {$product_entity_id},
                                                1,
...
function purge_stale_deactivated()
{
    ...
    {
        //rdi_deactivated_date
        $sql = "select distinct catalog_product_entity_datetime.value, entity_id from catalog_product_entity_datetime where (catalog_product_entity_datetime.value < ADDDATE(NOW(), INTERVAL -{$deactivated_delete_time} DAY)) and catalog_product_entity_datetime.value is not null and catalog_product_entity_datetime.attribute_id = {$deactivated_date_attribute_id} limit 10";
        $delete_ids = $cart->get_db()->cells($sql, 'entity_id');
        ...
        if(is_array($delete_ids))
        {
            foreach($delete_ids as $id)
            {
                ...
                {
                    echo "<br/>Can't delete product w/ id: $id";
                }
