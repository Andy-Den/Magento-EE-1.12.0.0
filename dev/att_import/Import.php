	...
	protected function populateOptionTable($attribId) {
		...
		for($indx = 1; $indx < count ( $csv ); $indx ++) {
			...
			if (isset ( $row ) && count ( $row ) > 0) {
				...
				$select = $read->select ()->from ( array ('vals' => $optionValueTable ) )->join ( array ('opt' => $optionTable ), 'opt.option_id=vals.option_id' )->where ( 'opt.attribute_id=?', $attribId );
				$select = $select->where ( 'vals.value in ' . $whereParam );
				...
				if (! $intOptionId) {
					$data = array ('attribute_id' => $attribId, 'sort_order' => isset ( $option ['order'] [$optionId] ) ? $option ['order'] [$optionId] : 0 );
					try {
				...
				foreach ( $values as $storeId => $value ) {
					if (! empty ( $value ) || strlen ( $value ) > 0) {
						...
						foreach ( $optionValData as $valData ) {
							if (( int ) $valData ['option_id'] === $intOptionId && ( int ) $valData ['store_id'] === $storeId) {
